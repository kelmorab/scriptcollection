...
for i,f in enumerate(files):
    ...
    resprereg = TwoDplot("resprereg","",[100,0,300],[100,-1,1])
    respostreg = TwoDplot("respostreg","",[100,0,300],[100,-1,1])
    outputdif_full = TwoDplot("outputdif_full","",[binning_outputdiv_target[i][0],binning_outputdiv_target[i][1],binning_outputdiv_target[i][2]],[binning_outputdiv_div[i][0],binning_outputdiv_div[i][1],binning_outputdiv_div[i][2]])
    outputdif_zoom = TwoDplot("outputdif_zoom","",[binning_outputdiv_zoom_target[i][0],binning_outputdiv_zoom_target[i][1],binning_outputdiv_zoom_target[i][2]],[binning_outputdiv_zoom_div[i][0],binning_outputdiv_zoom_div[i][1],binning_outputdiv_zoom_div[i][2]])

    
    
    resprereg.setAxisTitle("p_{T, Parton} - p_{T, Jet} / p_{T, Jet}","p_{T, Jet}")
    respostreg.setAxisTitle("p_{T, Parton} - p_{T, Reg} / p_{T, Reg}","p_{T, Reg}") 
    outputdif_full.setAxisTitle("Output-Target Diviation","Target")
    outputdif_zoom.setAxisTitle("Output-Target Diviation","Target")
    


    histolist = [resprereg,respostreg,outputdif_full,outputdif_zoom]

    resprereg.projecttoTwoDplot(tree,"(Jet_MatchedPartonPt - Jet_Pt)/Jet_Pt:Jet_Pt","","")
    respostreg.projecttoTwoDplot(tree,"(Jet_MatchedPartonPt - ("+regpt+"))/("+regpt+"):("+regpt+")","","") 
    outputdif_full.projecttoTwoDplot(tree,"( BDTG - "+target+") : "+target,"","")
    outputdif_zoom.projecttoTwoDplot(tree,"( BDTG - "+target+") : "+target,"","")
    ...
    for histo in histolist:
        histo.WriteTwoDplot(c1,"ttbar",True,None,pdfout)
